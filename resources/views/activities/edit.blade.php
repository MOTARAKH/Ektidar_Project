<x-app-layout>
    ...
    <div class="container mx-auto p-6 bg-white shadow-md rounded-lg mt-2" dir="rtl">
        ...
        <form action="{{ route('activities.update', $activity->id) }}" method="POST">
            ...
            <!-- Type input -->
            <div class="mb-4">
                <label for="type" class="block text-sm font-medium text-gray-700">عنوان النشاط</label>
                <input type="text" id="type" name="address" value="{{ $activity->address }}" required
                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
            </div>

            <!-- Participating Parties input -->
            <div class="mb-4">
                <label for="ParticipatingParties" class="block text-sm font-medium text-gray-700">الجهات  الخارجية المشاركة</label>
                <input type="text" id="ParticipatingParties" name="side" value="{{ $activity->side }}" required
                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
            </div>
            ...
            <div id="descriptions-container" class="mb-6">
                ...
                @foreach ($activity->descriptions as $index => $description)
            ...
            <!-- Back button -->
            <a href="{{ route('activities.index') }}" class="ml-4 bg-gray-500 text-white p-2 rounded hover:bg-gray-600">
                العودة إلى النماذج
            </a>
        </form>
    </div>
    ...
    <script>
        let descriptionCount = {{ $activity->descriptions->count() }};
    </script>
       <script>
